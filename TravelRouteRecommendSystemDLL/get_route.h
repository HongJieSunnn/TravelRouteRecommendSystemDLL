...
namespace GetRouteNameSpace
{
	...
	*/
	...
	auto differentTypeVehicleGreater_TimeFirst = [](vector<Vehicle*> route1, vector<Vehicle*> route2)->bool
	{
		...
		if (route1[0]->getVehicleType() == HSRC_TYPE && route2[0]->getVehicleType() == HSRC_TYPE)
		{
			if (route1.size() == 1)
			{
				time1 -= time1 / 8;
...
public:
	...
	* route_size:所有线路 例如 福州-南京有两个线路 分别是 福州南-南京南 福州南-杭州东&&杭州东-南京南 那么route_size=3
	*/
	GetRouteNameSpace::GetRouteResultStatue getRouteResultsOneGroup(RouteResult*& route_result, int& route_size);
