...
* 
*/
extern "C" __declspec(dllexport)void GetRecommendationsOneGroup(UserRequirementFromCSharp requirement, RouteResult* & routes, int& level,char*& error_code,char*& error, char*& remark, int& route_group_nums, int& route_size_in_routes, int*& first_route_of_one_size_array);
/*
释放空间 
* 参数看上一个
* 可能需要把每个vehicle* 都释放 
...
extern "C" __declspec(dllexport)void FreeMemoryOneGroup(RouteResult * &routes, int*& first_route_of_one_size_array, int route_size_in_routes);
#endif // !TOBACKEND_H
